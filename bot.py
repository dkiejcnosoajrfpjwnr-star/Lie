...
def _ydl_attempts(url: str) -> list:
    ...
    if any(d in u for d in ["youtube.com", "youtu.be"]):
        return [
            _build_ydl_opts(url, {"extractor_args": {"youtube": {"player_client": ["web"]}}}),
...
def _yt_audio_download_sync(video_id: str) -> dict:
    """يحمّل أفضل صوت من يوتيوب ويحوّله لـ MP3."""
    ...
    with yt_dlp.YoutubeDL(opts) as ydl:
        info = ydl.extract_info(url, download=True)
        if not info:
            return {"success": False, "error": "فشل الاستخراج"}
        ...
        return {"success": False, "error": "لم يُنشأ الملف"}
